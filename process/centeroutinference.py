...
model_config["padding_mode"] = str(
    PROPRIOCEPTION_CONFIG.get("temporal_padding_mode", "zeros")
).lower()
if model_config["padding_mode"] not in {"zeros", "replicate", "reflect"}:
    raise ValueError(
        "proprioception.temporal_padding_mode must be zeros, replicate, or reflect"
    )
...
for direction, sp_path in spindle_files:
    ...
    # The pretrained network's XYZ labels were generated by this analytic FK,
    # not from the OpenSim Handle marker. Keep evaluation in the checkpoint's
    # training coordinate convention. OpenSim marker positions remain in the
    # upstream artifact for physical-path diagnostics, but are not equivalent
    # ground-truth labels for this network.
